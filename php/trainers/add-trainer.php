<?php
$page_title = 'Thêm huấn luyện viên';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($full_name === '' || $specialty === '') {
        ...
        $upload = upload_trainer_avatar_file($_FILES['avatar'], $upload_dir);
    ...
    if ($error === '') {
        $stmt = $conn->prepare('
            INSERT INTO trainers
                (full_name, avatar, specialty, experience_years, phone, email, bio, rating, total_members, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ');
        $stmt->bind_param(
            'sssisssdis',
        ...
        if ($stmt->execute()) {
            $stmt->close();
            header('Location: ' . $base_path . 'trainers.php?add=success');
            exit;
        }
        ...
        $error = 'Thêm HLV thất bại: ' . $stmt->error;
        $stmt->close();
    }
}
?>
  ...
  <div class="main-content flex-grow-1">
    ...
    <div class="container-fluid p-4">
      <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 class="fw-bold mb-1">Thêm huấn luyện viên</h2>
          <p class="text-muted mb-0">Tạo hồ sơ HLV để hiển thị ở khu vực user và nhận lịch đặt.</p>
        </div>
          ...
          <form method="POST" enctype="multipart/form-data">
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input type="text" name="specialty" class="form-control" value="<?php echo h($_POST['specialty'] ?? ''); ?>" placeholder="Ví dụ: Tăng cơ, giảm mỡ, cardio..." required>
              </div>
              <div class="col-md-3">
                <label class="form-label">Kinh nghiệm (năm)</label>
                <input type="number" name="experience_years" class="form-control" min="0" value="<?php echo h($_POST['experience_years'] ?? '0'); ?>">
              </div>
              <div class="col-md-3">
                <label class="form-label">Đánh giá mặc định</label>
                <input type="number" name="rating" class="form-control" min="0" max="5" step="0.1" value="<?php echo h($_POST['rating'] ?? '5.0'); ?>">
              </div>
              <div class="col-md-3">
                <label class="form-label">Hội viên đã hỗ trợ</label>
                <input type="number" name="total_members" class="form-control" min="0" value="<?php echo h($_POST['total_members'] ?? '0'); ?>">
              </div>
              <div class="col-md-3">
              ...
              <div class="col-md-8">
                <label class="form-label">Ảnh HLV</label>
              ...
              <div class="col-12">
                ...
                <textarea name="bio" class="form-control" rows="5" placeholder="Mô tả kinh nghiệm, thế mạnh và phong cách huấn luyện..."><?php echo h($_POST['bio'] ?? ''); ?></textarea>
              </div>
              <div class="col-12 mt-4">
                <button type="submit" class="btn btn-primary">
                  <i class="bi bi-save me-1"></i>Lưu HLV
                </button>
