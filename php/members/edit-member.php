...
$result_packages = $conn->query("SELECT id, package_name, duration_months FROM packages WHERE status = 'active' ORDER BY id DESC");
if ($result_packages) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($error === '') {
        ...
        try {
            $stmt = $conn->prepare('UPDATE members SET full_name = ?, gender = ?, phone = ?, email = ?, date_of_birth = ?, address = ?, package_id = ?, start_date = ?, end_date = ?, status = ? WHERE id = ?');
            $stmt->bind_param('ssssssisssi', $full_name, $gender, $phone, $email, $date_of_birth, $address, $package_id, $start_date, $end_date, $status, $id);
            $stmt->execute();
            $stmt->close();

            $stmt_latest_package = $conn->prepare('SELECT id FROM member_packages WHERE member_id = ? ORDER BY id DESC LIMIT 1');
            $stmt_latest_package->bind_param('i', $id);
            $stmt_latest_package->execute();
            $latest_package = $stmt_latest_package->get_result()->fetch_assoc();
            $stmt_latest_package->close();

            if ($latest_package) {
                $member_package_id = (int) $latest_package['id'];
                $stmt_update_package = $conn->prepare('UPDATE member_packages SET package_id = ?, start_date = ?, end_date = ?, status = ? WHERE id = ?');
                $stmt_update_package->bind_param('isssi', $package_id, $start_date, $end_date, $member_package_status, $member_package_id);
                $stmt_update_package->execute();
                $stmt_update_package->close();
            } else {
                $stmt_insert_package = $conn->prepare('INSERT INTO member_packages (member_id, package_id, start_date, end_date, status) VALUES (?, ?, ?, ?, ?)');
                $stmt_insert_package->bind_param('iisss', $id, $package_id, $start_date, $end_date, $member_package_status);
                $stmt_insert_package->execute();
                $stmt_insert_package->close();
            }
            ...
            $conn->rollback();
            $error = 'Cập nhật hội viên thất bại: ' . $e->getMessage();
        }
    }
            ...
            <form method="POST" action="">
              ...
              <div class="row g-3">
                ...
                <div class="col-md-6">
                  ...
                  <input type="date" name="date_of_birth" class="form-control" value="<?php echo htmlspecialchars($member['date_of_birth'] ?? ''); ?>">
                </div>
                ...
                <div class="col-md-6">
                  ...
                  <select name="package_id" id="package_id" class="form-select" required>
                    ...
                    <?php foreach ($packages as $package): ?>
                      <option value="<?php echo (int) $package['id']; ?>" data-duration="<?php echo (int) $package['duration_months']; ?>" <?php echo ((int) ($member['package_id'] ?? 0) === (int) $package['id']) ? 'selected' : ''; ?>>
                ...
                <div class="col-md-4">
                  ...
                  <input type="date" name="start_date" id="start_date" class="form-control" value="<?php echo htmlspecialchars($member['start_date'] ?? ''); ?>" required>
                </div>

                <div class="col-md-4">
                  <label class="form-label">Ngày kết thúc</label>
                  <input type="date" id="end_date_display" class="form-control" readonly value="<?php echo htmlspecialchars($member['end_date'] ?? ''); ?>" placeholder="Tự động tính">
                </div>
